//! Gateway ↔ engine ↔ client OPE wire protocol types.
//!
//! Source of truth for Confidential AI envelope metadata, engine-plane paths,
//! traffic class, and NDJSON stream frames. TypeScript consumers use
//! `bindings/typescript/ope-protocol` (`@teechat/ope-protocol`).
//!
//! Crypto stays in `ope-e2e` / `ope-envelope` / `ope-ffi` — this crate is types + codec only.
